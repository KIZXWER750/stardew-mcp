...
## 1.11.1 일반 야생 나무·밑동 제거

| 함수 | 주요 입력 | 역할 |
|---|---|---|
| `remove_wild_trees` | 공통 Farm 영역, `max_trees`(기본 3, 1~12), `preserve_young_trees`(기본 false), 에너지·시간 제한 | 영역 안의 가까운 일반 야생 나무를 성장 단계와 관계없이 선택하고 각 나무를 밑동까지 제거한 뒤 원래 TerrainFeature가 사라졌는지 검증 |
