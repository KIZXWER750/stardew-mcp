...
## 1.11.0 일반 야생 나무·밑동 제거

| 함수 | 주요 입력 | 역할 |
|---|---|---|
| `remove_wild_trees` | 공통 Farm 영역, `max_trees`(기본 3, 1~12), `include_saplings`(기본 false), 에너지·시간 제한 | 영역 안의 가까운 일반 야생 나무를 선택하고 각 나무를 밑동까지 도끼로 제거한 뒤 원래 TerrainFeature가 사라졌는지 검증 |

대상은 `Tree` TerrainFeature만입니다. `FruitTree`, 덤불, 작물, 건물, 상자·기계·배치 물체, 큰 밑동·통나무 같은 resource clump는 제외합니다. 기존 `clear_area`와 `prepare_plot`은 여전히 나무를 보호합니다. 나무 제거는 사용자 요청에 나무 제거가 포함된 경우에만 호출합니다.

각 대상은 동서남북 한 칸에서만 작업하고, 도끼가 없거나 접근할 수 없거나 에너지·시간이 부족하면 부분 진행 상태를 반환합니다. 한 대상당 타격은 40회로 제한됩니다. 성목이 밑동으로 변할 때 낙하 상태가 정리되도록 추가 대기한 뒤 밑동 작업을 이어갑니다. 완료는 도구 사용 횟수가 아니라 원래 나무 좌표에서 일반 Tree 지형이 사라진 상태입니다. 드롭 수집은 별도 향후 함수입니다.
