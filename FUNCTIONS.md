...
- 수면 완료는 다음 날짜·FarmHouse·조작 가능 상태에 더해 실제 위치 변화와 `6:00` 이후 시계 진행을 모두 요구합니다.
- 입력으로 위치가 변한 뒤에는 추가 이동을 멈추고 게임 시간이 `6:10` 이상으로 진행될 때까지 확인합니다.
- 다음 날 첫 단계가 농사 작업이고 플레이어가 FarmHouse 안에 있으면 계획 단계를 임대하기 전에 `exit_house`를 실행합니다.
- Farm 전용 함수의 위치 사전 검사는 잘못된 위치에서 게임 행동이나 멱등성 기록을 만들지 않습니다. Farm 도착 확인 뒤 같은 요청을 다시 사용할 수 있습니다.
