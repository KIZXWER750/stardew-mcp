...
if ($version -ne '1.19.8') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.8-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.8 installed. Dead crops are removed before planting/watering, morning wake-up sends one left click before clock verification, and the left plan HUD colors completed steps yellow. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
