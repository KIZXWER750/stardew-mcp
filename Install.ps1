...
if ($version -ne '1.17.0') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.17.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.17.0 installed. Phase 2 economic observations, crop-profit candidates, capability reporting, and verified empty-soil restoration are enabled. Strategy selection and autonomous earning remain future phases. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals.'
