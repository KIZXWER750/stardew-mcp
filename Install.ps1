...
if ($version -ne '1.10.0') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.10.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.10.0 installed. Added daily status, food recovery, verified return-home and sleep tools. Default: OpenAI gpt-5.6-luna / medium (fixed). Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
