...
if ($version -ne '1.19.9') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.9-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.9 installed. Answered goal questions are deduplicated before UI presentation, saved duplicate pending questions are suppressed on load, and planners ask about the actual unresolved condition after authorization. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
