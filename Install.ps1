...
if ($version -ne '1.19.2') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.2-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.2 installed. Existing planted crops are now planned separately from new farming, including bounded watering, harvest, sale and multi-day resume. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
