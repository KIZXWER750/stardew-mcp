...
if ($version -ne '1.19.1') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.1-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.1 installed. Empty earning sources now use the dedicated question UI, and early/closed Pierre stages persist a timed auto-resume. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
