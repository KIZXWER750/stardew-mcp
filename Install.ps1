...
if ($version -ne '1.14.1') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.14.1-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.14.1 installed. Mature-tree selection now requires observed growth evidence and zero eligible targets are blocked. Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
