...
if ($version -ne '1.19.5') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.5-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.5 installed. Persistent plans are bounded, oversized tool observations recover without terminating the goal, and OpenAI receives the full goal execution rules. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
