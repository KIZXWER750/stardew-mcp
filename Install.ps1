...
if ($version -ne '1.14.0') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.14.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.14.0 installed. Overnight confirmations, per-tree stump/drop completion, persistent tree resume and farthest-first shortest clearing routes are included. Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
