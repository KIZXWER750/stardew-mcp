...
if ($version -ne '1.19.7') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.7-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.7 installed. Morning verification now requires real movement and clock advancement, and wrong-location farm calls remain safely retryable after reaching Farm. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
