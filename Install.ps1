...
if ($version -ne '1.14.3') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.14.3-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.14.3 installed. Tree observations now distinguish twigs, young trees, mature trees, and stumps and describe the verified removal sequence. Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
