...
if ($version -ne '1.19.6') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.6-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.6 installed. Explicit daily sleep permission is persisted; future-day plan checkpoints schedule a separate return-home/sleep AI run and resume the plan after the next morning is verified. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
