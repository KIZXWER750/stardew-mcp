...
if ($version -ne '1.11.2') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.11.2-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.11.2 installed. Tree jobs now use stage-specific hit limits, observe real loose debris, clear safe access obstacles, and verify drop pickup. Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
