...
$copilot = $null
if (!$BuildOnly) { $copilot = (Get-Command copilot -CommandType Application -ErrorAction Stop | Select-Object -First 1).Source }
$version = (Get-Content "$src\mod\StardewMCP\manifest.json" -Raw | ConvertFrom-Json).Version
if ($version -ne '1.8.0') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.8.0-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.8.0 installed. Load a save with SMAPI; F6 opens commands, F7 cancels (unless customized).'
