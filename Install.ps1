...
if ($version -ne '1.10.1') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.10.1-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.10.1 installed. Fixed standard Farm to FarmHouse route discovery. Default: OpenAI gpt-5.6-luna / medium (fixed). Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
