...
if ($version -ne '1.19.4') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.4-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.4 installed. Morning automation waits for the wake-up transition to restore control before sending normal movement input and verifying that the 6 AM clock can advance. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
