...
if ($version -ne '1.19.3') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.19.3-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.19.3 installed. GameData-backed planning works in the split game assembly, and future-day checkpoints remain GOAL WAITING for automatic resume. Restart Steam/SMAPI; F6 opens commands or a pending question, F7 pauses active goals and plans.'
