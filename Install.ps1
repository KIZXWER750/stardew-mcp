...
if ($version -ne '1.14.2') { throw "Wrong source version: $version" }
...
$backup = Join-Path (Split-Path $src -Parent) ('StardewMCP-before-1.14.2-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host '1.14.2 installed. The 6 AM new-day input gate now receives and verifies normal movement input. Restart Steam/SMAPI; F6 opens commands, F7 cancels.'
