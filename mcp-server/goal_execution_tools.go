...
WAITING means today's checkpoint succeeded and the long-term goal remains active. Stop this run with a final line beginning GOAL WAITING: and briefly name the next persisted day/time step. Never call it TASK_BLOCKED. The in-game host automatically resumes a due persisted step on a later day. If the saved goal policy explicitly allows daily return-home and sleep, the host schedules a separate AI run to return home and sleep, then schedules another plan run after the morning is verified. Do not perform those actions in the current plan run, and never invent permission that is absent from the saved policy.
