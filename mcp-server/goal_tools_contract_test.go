...
func TestLongTermGoalContract(t *testing.T) {
	...
	text := string(tools)
	for _, required := range []string{"create_long_term_goal", "verify_long_term_goal", "request_goal_input", "does not autonomously select or execute", "dedicated response window"} {
