...
func TestMoveCompletionDoesNotUseOldCache(t *testing.T) {
    ...
    resp := <-ch
    if c.GetState().Player.X != 61 { t.Fatal("state was not updated before tool returned") }
    if !strings.HasPrefix(verifyMoveCompletion(resp,"Farm",61,19),"Arrived") { t.Fatal("actual arrival rejected") }
}
...
func TestMoveCompletionRejectsUnverifiedResults(t *testing.T) {
    ...
    for _,resp := range cases {
        if !strings.HasPrefix(verifyMoveCompletion(resp,"Farm",61,19),"TASK_BLOCKED:") { t.Fatal("unverified movement accepted") }
    }
}
